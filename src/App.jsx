...
const App = () => {
  ...
  return (
    <div>
      {isMobileMenuOpen && (<div className="fixed inset-0 bg-black/20 z-10"></div>)}
      ...
      <div className="w-72 transition-transform duration-500 ease-in-out rounded-md h-32 fixed left-10 z-40 bottom-5 bg-white" style={{ boxShadow: "0 0 10px rgba(0, 0, 0, 0.2)", transform: flashCardOpen ? "translateX(0)" : "translateX(calc(-100% - 2.5rem))" }}>
        <div className="flex w-full items-center">
          ...
          <div className="py-4 flex flex-col gap-2 w-full items-center">
            <Link to="/product-details" className="text-[16px] leading-[20px] text-[#001430] font-semibold transition-colors duration-300 hover:text-[#00bbae]">{product.name}</Link>
            <p className="text-[16px] leading-[20px] text-[#001430] font-semibold">{product.price}</p>
          </div>
        </div>
      </div>
      {
        isShowTopButton && (
          <div onClick={scrollToTop} className="fixed bottom-5 z-50 right-5 w-10 h-10 rounded-full bg-[#f88e0f] flex items-center justify-center cursor-pointer transition-transform duration-300 hover:scale-110">
