      ...
      <div className="w-full bg-white lg:px-12 sm:px-20 px-5">
        <div className="w-full bg-[#002cc7] h-full mx-auto my-10 rounded-3xl flex md:flex-row flex-col justify-center items-center">
          <div className="flex flex-col gap-5 sm:gap-10 w-full px-8 py-10 md:w-1/2">
            <p className="text-white text-[32px] md:text-[38px] leading-[48px] md:leading-[57px] font-bold">Get 25% discount in all kind of <br />super hero theme</p>
            <button onMouseEnter={() => setButtonHovered(true)} onMouseLeave={() => setButtonHovered(false)} className="flex gap-2 w-44 bg-orange-500 transition-all duration-300 hover:bg-orange-400 cursor-pointer rounded-full items-center justify-center p-3">
              <p className="text-[16px] font-semibold text-white leading-[24px]">See Collection</p>
              <ArrowRight className={`w-5 h-5 text-white transition-transform duration-300 ${buttonHovered ? "translate-x-1" : ""}`} />
            </button>
          </div>
