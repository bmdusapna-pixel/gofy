...
const ProductsCollection = ({ color }) => {
  ...
  return (
    <div className="w-full bg-[#f8f9fa]" style={{ boxShadow: "inset 0 8px 8px -8px rgba(0, 0, 0, 0.5)"}}>
                    ...
                    <Link to={`/product-details/${product.url}`} className="bg-white rounded-md p-3 flex flex-col w-full cursor-pointer relative" onMouseEnter={() => setHoveredId(product._id)} onMouseLeave={() => setHoveredId(null)}>
                      <div className="flex relative">
                        ...
                        <div className="w-full h-72 sm:h-[280px] flex items-center justify-center">
                          <img src={product.images[0]} alt={product.name} className={`absolute top-0 left-0 w-72 h-full ease-in-out ${ isHovered ? "opacity-0 scale-105 duration-300" : "opacity-100 scale-100 duration-100"}`} draggable={false}/>  
                          <img src={product.images[1]} alt={product.name + " hover"} className={`absolute top-0 left-0 w-72 h-full ease-in-out ${isHovered ? "opacity-100 scale-100 duration-300" : "opacity-0 scale-95 duration-100"}`} draggable={false} />
                        </div>
                      </div>
                      {
                      ...
                      <div className="flex flex-col gap-2 px-2">
                        ...
                        <div className="flex gap-2 items-center">
                          <p className="text-[16px] leading-[24px] line-through font-semibold text-[#212529]">₹ 20</p>
                          <p className="text-[18px] leading-[27px] font-semibold text-pink-600">₹{product.price}</p>
                        </div>
                        <button onClick={(event)=>addProductToCart(product, event)} className="text-[16] leading-[24px] px-2 w-36 font-semibold cursor-pointer py-2 rounded-md hover:bg-[#00bbae] bg-[#e9ecef] hover:text-white transition duration-300">ADD TO CART</button>
                      </div>
                    </Link>
                  </SwiperSlide>
                );
