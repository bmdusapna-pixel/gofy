        ...
        <div className="flex gap-5 w-full">
          <div className="lg:w-1/4 lg:flex hidden flex-col gap-5">
            ...
            <div className="border-gray-200 border-[2px] bg-white rounded-2xl p-6 flex flex-col gap-5">
              <div className="flex flex-col gap-2 w-full">
                <p className="text-[18px] leading-[27px] font-semibold text-black">Products</p>
                <hr className="w-14 rounded-2xl bg-[#f88e0f] h-1 border-none" />
              </div>
              <div className="flex flex-col gap-2 w-full">
                {
          ...
          <div className="lg:w-4/5 w-full flex flex-col gap-5">
            <div className="flex justify-between w-full items-center bg-white rounded-xl p-4">
              <p className="text-[#69778a] text-[18px] leading-[24px] font-semibold lg:block hidden">Showing all {productItems.length} result{productItems.length !== 1 ? 's' : ''}</p>
            ...
            <div className="w-full grid md:grid-cols-4 grid-cols-1 sm:grid-cols-2 gap-3">
              {
                productItems?.map((product, index) => (
                  <Link to={product.category === "clothes" ? `/products/clothes/${product.url}` : `/products/toys/${product.url}`} key={index} onMouseEnter={()=>setProductHoverd(product._id)} onMouseLeave={()=>setProductHoverd(null)} className="w-full cursor-pointer border border-gray-200 rounded-md p-3 shadow-sm hover:shadow-xl bg-white flex flex-col gap-2 lg:gap-3 transition-shadow duration-300 group">
