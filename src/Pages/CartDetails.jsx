...
const CartDetails = () => {
  const { openCart, setOpenCart, totalItems, cartItems, formSubmit, increaseQuantityFromCart, decreaseQuantityFromCart, totalPrice, removeProductFromCart } = useContext(CartContext);
      ...
      <div className="w-full lg:px-12 px-5 mx-auto flex lg:flex-row flex-col gap-5">
            ...
            (
              <div className="flex flex-col w-full h-full">
                <div className="flex w-full items-center justify-between bg-[#f8f9fa] rounded-tl-2xl rounded-tr-2xl px-8 py-3">
                  <p className="text-[16px] leading-[24px] text-[#001430] font-semibold text-center hidden md:block w-[60px]">Remove</p>
                ...
                </div>
              {
                cartItems.map((item, index) => (
                  <div key={item._id} className={`flex w-full items-center justify-between relative bg-[#edfbfa] ${index === cartItems.length - 1 ? "rounded-bl-2xl rounded-br-2xl" : "border-b-[1px]"} px-8 py-3`}>
                    <div onClick={() => removeProductFromCart(item)} className="hidden md:flex items-center justify-center w-[60px] h-[32px] cursor-pointer">
                    ...
                    <div className="hidden md:flex items-center justify-center w-[100px]">
                      <img src={item.image} alt={item.name} className="object-cover w-full rounded-md" />
                    </div>
            ...
            (
              <div className="flex flex-col w-full gap-5">
                <div className="border border-[#00bbae] p-4 rounded-xl flex gap-2 items-center w-full bg-[#edfbfa]">
                  ...
                  <p className="text-[14px] text-[#2f2f2f] leading-[21px] font-normal">Your cart is currently empty</p>
                </div>
        ...
        <div className="w-full lg:w-2/6 flex flex-col gap-5">
          <div className="w-full md:p-10 p-5 flex flex-col gap-5 bg-white rounded-2xl">
            <p className="text-[28px] leading-[42px] font-semibold text-black">Apply coupon</p>
            <form onSubmit={submitFormCoupon} className="w-full flex-col gap-3 flex">
              <input required name="couponCode" onChange={(event) => setCouponeCode(event.target.value)} type="text" className="w-full rounded-2xl border border-gray-200 p-3 focus:outline-none focus:border-[#00bbae] transition duration-300" placeholder="Coupon Code" />
          ...
          <div className="w-full md:p-10 p-5 flex flex-col gap-3 bg-white rounded-2xl">
            <p className="text-[28px] leading-[42px] font-semibold text-black">Cart totals</p>
            <div className="w-full flex-col gap-3 flex">
              <div className="flex justify-between items-center w-full">
                <p className="text-[18px] leading-[27px] font-medium text-[#69778a]">Subtotal</p>
                <p className="text-[18px] leading-[27px] font-medium text-[#212121]">₹ {totalPrice}</p>
              </div>
              <div className="flex justify-between items-center w-full">
                <p className="text-[20px] leading-[30px] font-semibold text-black">Total</p>
                <p className="text-[20px] leading-[30px] font-semibold text-black">₹ {totalPrice}</p>
              </div>
            </div>
          </div>
        </div>
